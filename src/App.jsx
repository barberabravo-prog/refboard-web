  ...
  return (
    ...
    >
      <div style={{
        ...
        {card.image && !imgError ? (
          <img src={card.image} alt="" onError={() => setImgError(true)}
            style={{ width: "100%", height: "100%", objectFit: "cover" }}
            draggable={false} />
        ) : (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            <img src={"https://www.google.com/s2/favicons?domain=" + card.domain + "&sz=64"}
              alt="" style={{ width: 28, height: 28, opacity: 0.4 }} draggable={false} />
      ...
      <div style={{ padding: "10px 12px 12px" }}>
        <p style={{
          margin: "0 0 4px", fontSize: 12, fontWeight: 600, color: "#111",
          lineHeight: 1.4,
          display: "-webkit-box", WebkitLineClamp: 2,
          WebkitBoxOrient: "vertical", overflow: "hidden",
        }}>{card.title || card.domain}</p>
        {card.caption && (
          <p style={{
            margin: "0 0 8px", fontSize: 11, color: "#666",
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            ...
            style={{
              background: "#111", border: "none", borderRadius: 5,
...
export default function RefBoard() {
  ...
  const addCard = async () => {
    ...
    const card = {
      ...
      board_id: "default", url,
      title: meta.title, description: meta.description,
      image: meta.image, domain, caption: "",
      x: -offset.x / scale + 100 + Math.random() * 300,
      y: -offset.y / scale + 100 + Math.random() * 200,
    };
  ...
  const handleMouseMove = (e) => {
    ...
    if (draggingCard && cardDragStart.current) {
      ...
      setCards(prev => prev.map(c =>
        c.id === draggingCard ? { ...c, x: cardDragStart.current.cardX + dx, y: cardDragStart.current.cardY + dy } : c
      ));
    }
  };
  ...
  const zBtn = {
    ...
    color: "#666", fontSize: 16, width: 28, height: 28, cursor: "pointer",
    display: "flex", alignItems: "center", justifyContent: "center",
  };
    ...
    <>
      ...
      >
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button onClick={() => { setOffset({ x: 0, y: 0 }); setScale(1); }}
              style={{ background: "transparent", border: "1px solid #ddd", borderRadius: 6, color: "#888", fontSize: 11, padding: "4px 10px", cursor: "pointer" }}>
              Reset view
            </button>
          </div>
        </div>
        ...
        <div style={{
          ...
          transformOrigin: "0 0", position: "absolute", top: 0, left: 0, width: 0, height: 0,
        }}>
          {cards.map(card => (
        ...
        <div style={{
          ...
          <input
            ...
            onKeyDown={e => e.key === "Enter" && !loading && addCard()}
            placeholder="Pega un link: instagram.com/p/...  tiktok.com/...  pinterest.com/..."
            style={{
          ...
          <button
            ...
            style={{
              ...
              opacity: (!inputUrl.trim() && !loading) ? 0.3 : 1,
              whiteSpace: "nowrap", minWidth: 90, fontFamily: "inherit",
            }}
          >{loading ? "cargando..." : status || "Añadir"}</button>
        </div>
      </div>
    </>
  );
}
ENDOFFILEcat > ~/refboard-web/src/App.jsx << 'ENDOFFILE'
import { useState, useEffect, useRef, useCallback } from "react";
import { createClient } from "@supabase/supabase-js";
